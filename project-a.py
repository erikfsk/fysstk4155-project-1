...
class machine_learning():
	...
	def manual(self,degree = None):
		data_points_x = self.data_points_x
		data_points_y = self.data_points_y
		data_points_z = self.data_points_z
		degree = self.degree if degree is None else degree

		#Manual learning solution
		xb = []
		...
		self.manual_z = zpredict.reshape(100,100)
		self.manual_XY = xb
	...
	def plot(self):
		...
		if scikit_z is not None:
			surf = ax.plot_surface(x, y, scikit_z, cmap=cm.Reds,
									linewidth=0, antialiased=False)
		if manual_z is not None:
			surf = ax.plot_surface(x, y, manual_z, cmap=cm.Oranges,
								linewidth=0, antialiased=False)
	...
	def get_errors(self):
		...
		if manual_z is not None:
			print("Manual MSE is %.6f" % self.MSE_error(manual_z,z))
			print("Manual R^2 is %.6f\n" % self.R2_error(manual_z,z))
	...
	def var(self):
		...
		sigma_2 = self.MSE_error(manual_z,z)
...
if __name__ == '__main__':
	...
	test = machine_learning(FrankeFunction,degree = 2)
	# test.scikit_Lasso()
	test.scikit()
	test.manual()
	test.get_errors()
